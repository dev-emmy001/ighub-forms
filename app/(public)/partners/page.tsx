...
export default function PartnersStatsPage() {
    ...
    return (
        <main className="min-h-screen bg-ighub-light text-ighub-black flex flex-col justify-between font-sans">
            <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-4xl w-full mx-auto">
                ...
                {!stats ? (
                    ...
                    <div className="w-full max-w-md bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-xs border border-gray-100/60 text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        <div>
                            <h1 className="text-2xl font-black text-ighub-black tracking-tight">
                                Partner Tracking Hub
                            </h1>
                            <p className="text-xs text-gray-500 mt-2 leading-relaxed max-w-xs mx-auto">
                                Enter your registered affiliate email below to check your clicks, sales metrics, and total payouts.
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleCheckStats} className="space-y-4">
                            ...
                            <Button
                                ...
                                fullWidth
                                disabled={isLoading}
                                className="font-bold py-3.5 rounded-2xl cursor-pointer shadow-md select-none flex justify-center items-center gap-2"
                            >
                                {isLoading ? (
                                    <>
                    ...
                    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-300">
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-3xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <div>
                                <span className="text-[10px] font-bold uppercase tracking-wider text-ighub-purple bg-ighub-light px-3 py-1 rounded-full border border-gray-200">
                                    Active Partner Profile
                                </span>
                                <h2 className="text-xl font-black text-ighub-black mt-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-3xs flex items-center gap-4 hover:shadow-2xs transition-all">
                                ...
                                <div>
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Total Link Clicks</span>
                                    <h3 className="text-3xl font-black text-ighub-black mt-0.5">{stats.promoter.clicks}</h3>
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-3xs flex items-center gap-4 hover:shadow-2xs transition-all">
                                ...
                                <div>
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Successful Sales</span>
                                    <h3 className="text-3xl font-black text-ighub-black mt-0.5">
                                        {stats.stats.successfulSales}
                                        <span className="text-xs text-gray-400 font-bold ml-1">/ {stats.stats.totalReferrals} regs</span>
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-3xs flex items-center gap-4 hover:shadow-2xs transition-all">
                                ...
                                <div>
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Payouts Earned</span>
                                    <h3 className="text-3xl font-black text-ighub-black mt-0.5">
                                        ₦{stats.stats.commissionEarned.toLocaleString()}
                                    </h3>
                                </div>
                            </div>
                        </div>

                        {/* Referral Link & Bank Details row */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Copy Referral Link Box */}
                            {stats.campaign && (
                                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-3xs flex flex-col justify-between space-y-4">
                                    <div>
                                        <h3 className="text-sm font-bold text-ighub-black flex items-center gap-1.5">
                                            <TrendingUp className="w-4 h-4 text-ighub-green" />
                                            Active Campaign Link
                                        </h3>
                                        <p className="text-xs text-gray-500 mt-1">
                                            Share this link with your audience. You will earn a 10% commission (₦{(stats.campaign.ticketPrice * 0.1).toLocaleString()} per sale) on each paid attendee referral for <strong>{stats.campaign.title}</strong>.
                                        </p>
                                    </div>
                            ...
                            {/* Payout Bank Details Box */}
                            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-3xs flex flex-col justify-between space-y-4">
                                <div>
                                    <h3 className="text-sm font-bold text-ighub-black flex items-center gap-1.5">
                                        <Building2 className="w-4 h-4 text-ighub-purple" />
                                        Registered Payout Method
                                    </h3>
                                    <p className="text-xs text-gray-500 mt-1">
                                        Commissions are calculated automatically and paid directly to your registered bank account details. If you wish to update this information, please contact administrator.
                                    </p>
                                </div>

                                <div className="bg-ighub-light p-4 rounded-2xl border border-gray-200 flex flex-col gap-2 justify-center flex-1">
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">
                                        Bank Transfer details
                                    </span>
                                    <span className="text-sm font-bold text-ighub-black leading-relaxed">
                                        {stats.promoter.bankDetails}
                                    </span>
                                </div>

                                <div className="pt-2 text-right text-[10px] font-semibold text-gray-400">
                                    Payout Status: <span className="text-ighub-green font-bold">Active & Verified</span>
                                </div>
                            </div>
                        </div>
                    </div>
